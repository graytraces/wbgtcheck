...
**"왜 정적 태그로 안 돌아가나" — 인젝션 전환의 프로덕션 비용은 측정했고, 알면서 받아들였다.**
정적 태그는 조건부로 만들 수 없어서(가드 자체가 불가능) 로더를 JS 인젝션으로 바꿨고, 그건
공짜가 아니다. `wbgtcheck.com` 오리진으로 dist를 서빙하고 gtag/js를 빈 스텁으로 fulfill해
(타이밍은 진짜, 구글로 가는 바이트는 0) 빌드당 15회 측정한 결과:

| | 정적 태그 | 인젝션 |
|---|---|---|
| 로더 요청 시작(median) | 4.7ms | 6.2ms (+1.5ms) |
| 요청 발행 순서 | gtag/js → 번들 → CSS | 번들 → CSS → **gtag/js** |

원인은 **preload scanner**다 — 정적 태그는 파서보다 앞서 스캔돼 큐에 들어가고, 인라인
스크립트가 append하는 요소는 그걸 이길 수 없다(3회 반복 재현). 수용한 근거: ① 두 버전 모두
`async`라 **렌더 차단 여부는 변하지 않는다** ② GA는 별도 오리진이라 연결 수립(실망 수십~수백 ms)이
2ms 스케줄링 차이를 압도한다 ③ preconnect는 원래 **장식이었다** — 빌드 산출물에서 스크립트
두 줄 위, 사이에 아무 리소스도 없이 놓여 있어 연결을 데울 시간 자체가 없었다.
정적 preconnect만 남기는 절충안은 **모든 로컬 페이지 로드마다 구글과 TLS 핸드셰이크**를
되살리므로 기각했다(2ms를 사려고 이 가드의 목적 절반을 파는 거래).
⚠️ 요청 **순서**는 테스트로 고정하지 않았다 — 방금 수용하기로 한 동작을 금지하는 테스트가 되므로.
